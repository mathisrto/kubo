...
function SceneContent() {
    ...
    useEffect(() => {
        ...
        if (selectedObject) {
            ...
            // Si aucun objet n'est sélectionné, nettoyer selectedMesh
            setSelectedMesh(undefined);
        }
    }, [selectedObject, meshes, setSelected]);

    useEffect(() => {
        // Nettoyer selectedMesh si l'objet sélectionné n'existe plus dans meshes
        if (selectedObject && !meshes[selectedObject]) {
            setSelectedMesh(undefined);
        }
    }, [selectedObject, meshes]);
